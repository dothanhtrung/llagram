...
enum Command {
    ...
    Clear(String),
    /// Enable or disable a skill: /skill <name> on|off
    Skill(String),
}
...
async fn handle_chat(app: Arc<App>, bot: Bot, msg: Message, text: String) -> ResponseResult<()> {
    ...
    let reply = if tools.is_empty() {
        ...
        tool_reply(&app, &bot, msg.chat.id, thinking_msg.id, messages, tools)
            .await
...
async fn current_thread_id(app: &App, chat_id: i64, user_id: i64) -> Option<i64> {
    ...
    db::get_current_thread_id(&app.pool, user_id)
        .await
        .ok()
        .flatten()
}
...
fn skill_list_text(enabled: &std::collections::HashSet<String>) -> String {
    ...
    for info in Registry::all() {
        let state = if enabled.contains(info.name) {
            "on"
        } else {
            "off"
        };
